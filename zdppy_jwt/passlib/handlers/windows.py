...
# site
# pkg
from zdppy_jwt.passlib.utils import to_unicode, right_pad_string
from zdppy_jwt.passlib.utils.compat import unicode
from zdppy_jwt.passlib.crypto.digest import lookup_hash
...
class lmhash(uh.TruncateMixin, uh.HasEncodingContext, uh.StaticHandler):
    ...
    def raw(cls, secret, encoding=None):
        ...
        from zdppy_jwt.passlib.crypto.des import des_encrypt_block
        MAGIC = cls._magic
...
class msdcc2(uh.HasUserContext, uh.StaticHandler):
    ...
    def raw(cls, secret, user):
        ...
        :returns: returns string of raw bytes
        """
        from zdppy_jwt.passlib.crypto.digest import pbkdf2_hmac
        secret = to_unicode(secret, "utf-8", param="secret").encode("utf-16-le")
        user = to_unicode(user, "utf-8", param="user").lower().encode("utf-16-le")
